...
1. 异常处理要颗粒化
2. 长事务如果回滚会出现过多回滚，需要拆分事务为分批的短事务
3. 如果有网络问题就需要进行重试，可以考虑使用 `Guava Retrying`，或者激进来说使用增量恢复
4. 性能优化上，不要过多使用 `for`，替换为批处理插入记录
5. 使用`Druid` 开源数据库连接池，同时利用观测工具来细致优化 `SQL`，包括内部的连接池问题

### 7.2.1.向题库中批量绑定题目

选中多个题目选项，跳出弹窗后批量使用事务来绑定题库。

### 7.2.2.向题库中批量解绑题目

选中多个题目选项，跳出弹窗后批量使用事务来解绑题库。
...
​	选中多个题目选项，跳出弹窗后批量使用事务来删除题目和题目题库。

### 7.2.5.热点数据缓存

- 对于获取题库的请求，如果  `5s` 内访问 `>= 10`，那么就开始使用本地缓存 `10min`
- 对题目做自动缓存，需要记录某条题目在一定时间内的访问次数（最难），然后根据阈值判断变为热点数据后开始在本地缓存

可以使用热点探查框架 `hotkey`，快速配置得到上述的需求。
...
1. **监控服务健康状态**，观察到某服务的错误率达到阈值后（接口失败到达 `20%`）
2. **系统激活熔断机制**，暂停对该服务的调用，避免消耗不必要的资源
3. **选用短期处理模式**
   1. 启用降低级别模式，如果允许服务被另外一个廉价的服务替代时可降级（但不是必须的）
   2. 启用快速失败模式，系统任意接口不再等待超时而是快速返回失败响应（用假数据欺骗）
4. **排查错误解决问题**...
5. **系统熔断恢复机制**，熔断并非永久状态。在一段时间后，熔断器会进入 **半开状态**，允许少量请求测试服务的健康情况。
   1. 如果恢复正常，熔断器将关闭，恢复正常服务调用
   2. 如果仍有问题，则继续保持熔断

> `try-catch` 机制就是一个最为简单的降级策略，但降级不仅仅是异常才触发，在一些资源接近短缺的场景也会触发。降级的更高级应用就是有损服务，例如动态检测不同微服务的资源占用，可以限制别的微服务，放宽某个特定微服务。
...
> - 流量规则：
>   - 流量策略：每个 IP 地址每分钟允许查看题目列表的次数不能超过 60 次。
>   - 阻塞操作：提示“访问过于频繁，请稍后再试”
> - 熔断规则：
>   - 熔断条件：如果接口异常率超过 10%，或者慢调用（响应时长 > 3 秒）的比例大于 20%，触发 60 秒熔断。
>   - 熔断操作：直接返回本地数据（缓存或空数据）

方便一些的做法就是使用控制台来设置流量机制规则、熔断机制规则、热参机制规则，然后使用样板代码来持久化为本地文件，代码中设置规则触发后的动作。

其中热参机制规则最好使用代码来实现，避免由于参数过多造成不必要的损耗。
